...
def circleCollision(c1, c2):
    if max(c1[0], c2[0]) < min(c1[0]+(constants.PERSON_RADIUS*2), c2[0]+(constants.PERSON_RADIUS*2)) and max(c1[1], c2[1]) < min(c1[1]+(constants.PERSON_RADIUS*2),c2[1]+(constants.PERSON_RADIUS*2)):
        return True
    else: 
        return False
